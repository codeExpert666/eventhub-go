...
改动后，普通 `error` 到 `*apperror.AppError` 的收敛归属 `internal/apperror`；`internal/http/validation` 只保留请求体、字段、query/path 参数校验错误构造。对外 API、HTTP 状态码、错误码、错误消息和响应 envelope 不变。
...
- 修改了什么
  ...
  - `internal/http/openapi_routes.go`
    ...
    - generated path/query 参数绑定错误最终调用 `validation.ParameterValidationError`。
  - `internal/http/handler/user/admin_validation.go`
    - 管理员 query/path 参数校验失败改为调用 `validation.ParameterValidationError`，删除包内重复的 `queryValidationError`。
...
- 关键设计原因
  ...
  - `internal/http/validation` 继续保留 HTTP 请求校验错误构造，避免把 `请求参数校验失败` 这类 HTTP 入口语义下沉到 service/domain。
  - `ParameterValidationError` 只接收 `FieldErrors`，不依赖 OpenAPI generated 参数错误类型；oapi-codegen 错误解析仍留在 `internal/http/openapi_routes.go`。
...
- 与 Java 版业务语义的对齐方式
  ...
  - Go 通过 `apperror.FromErrorOrInternal`、`validation.*ValidationError` 和 `response.WriteError` 显式完成等价语义。
...
- 方案 B：把整个 `internal/http/validation` 包重命名为 `internal/http/requesterror`。
  - 没有采用。当前包名仍适合表达 HTTP 请求校验，整体重命名会扩大 diff，但收益有限。
...
## 6. 已知限制
- 当前 `internal/http/validation` 仍保留 `validation` 包名，没有整体改成 `requesterror`。
  - 这是刻意选择：当前包继续服务 HTTP 请求校验，保留包名可以减少无意义 churn。
...
- 对后续 Go package、migration、sqlc、OpenAPI 或测试策略的影响
  ...
  - 新增 body 字段校验时继续调用 `validation.BodyValidationError`。
  - 新增 query/path 参数校验时继续调用 `validation.ParameterValidationError`。
