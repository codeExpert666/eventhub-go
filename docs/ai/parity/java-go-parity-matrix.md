...
| OpenAPI / Swagger | `OpenApiConfig.java`、Controller `@Operation/@Tag`、Springdoc dev/test/prod 开关、`OpenApiProductionSecurityTest`、Spring Security ADMIN 角色约束 | `api/openapi/eventhub.yaml`、`api/openapi/swagger/`、`api/openapi/oapi-codegen.yaml`、`api/openapi/openapi_policy_test.go`、`api/openapi/assets.go`、`api/openapi/gen/eventhub.gen.go`、`internal/config/config.go`、`internal/http/{router_contract_test.go,openapi_contract_test.go}`、`internal/http/handler/openapi`、`configs/*.env.example`、`Dockerfile`、`Makefile`、`redocly.yaml` | 已对齐 | Go 选择 spec-first，以 `api/openapi/eventhub.yaml` 作为契约源，`oapi-codegen` 通过 `api/openapi/oapi-codegen.yaml` 生成类型和 chi server interface；`api/openapi/assets.go` 仅汇总本地资源根目录和相对于该根目录的 YAML / Swagger UI 资源路径常量，调用方无需再拼接 Swagger 子目录和文件名；`internal/http` 契约测试各自通过 `runtime.Caller` 与 `kin-openapi` `LoadFromFile` 加载 `eventhub.yaml`，不再让生产包提供测试专用读取方法；Swagger UI 使用本地 `index.html` 和 vendored `swagger-ui-dist@5.17.14` CSS/JS，`/openapi.yaml` 从 `OPENAPI_ASSET_ROOT` 指向的本地目录读取，`OpenAPIHandler` 用一张静态资源白名单表统一服务 YAML、HTML、CSS、JS，并在 `OPENAPI_ENABLED=true` 时于构造阶段校验必需资源存在，不再依赖外部 CDN、Go embed、Go 字符串常量或 handler 自动目录搜索；`OPENAPI_ASSET_ROOT` 相对路径按进程当前工作目录解析，dev/test 默认启用 `/openapi.yaml` 和 `/swagger/*`，prod 默认关闭并返回 `COMMON-404`，容器默认 `OPENAPI_ASSET_ROOT=/app/api/openapi`。Go 不复刻 Springdoc 注解扫描和 `/v3/api-docs`/`/swagger-ui.html` 路径；Redocly CLI 通过 `redocly.yaml` 和 `make openapi-lint` 补充通用 OpenAPI 文档质量检查，覆盖 operationId、operationId 唯一、summary、tag 定义、nullable schema、schema example 和未使用组件提示；Go test policy 强制 operationId、summary/description、tags、业务 JSON 响应、2xx 最外层 `ApiResponse` envelope、非 2xx 集中引用 `components.responses`、组件错误响应统一使用 `ErrorResponse`、`ErrorResponse` 顶层复用 `ApiResponse`、admin `BearerAuth` 与精确 `x-required-roles: [ADMIN]`，并阻止非 admin operation 误标 `ADMIN`；`internal/http` 额外通过 `chi.Walk` 对比运行时 router 与 OpenAPI `paths`/`methods`，并用 `openapi3filter.ValidateResponse` 校验少量关键真实响应的 status、media type 和 body schema，覆盖 `/api/**` 与 `/actuator/**`，排除受 `OPENAPI_ENABLED` 控制的文档路由。当前审计见 `docs/ai/parity/current-auth-contract-checklist.md`。参考 design/implementation 015、018、021、022、023、024、026、027，ADR-0018、0019、0023。 |
...
| 容器化、部署配置与质量门禁 | Java `backend/Dockerfile`、`docker-compose.yml`、`application-dev/prod/test.yml`、prod OpenAPI hardening ADR | Go `Dockerfile`、`docker-compose.yml`、`configs/*.env.example`、`README.md`、`Makefile`、`.golangci.yml`、`.github/workflows/ci.yml`、`redocly.yaml` | 已对齐 | Go 多阶段 Dockerfile、MySQL/Redis/migrate/app compose、healthcheck、prod 默认关闭 OpenAPI、显式 migration job、`make quality`/`make quality-check`、固定 golangci-lint v2 版本、v2 配置格式、本机版本探测与 Docker fallback、GitHub Actions CI 已落地；CI 通过 `make quality-check`、`make openapi-lint`、`make generated-check` 和 Docker job 复用 Makefile 校验 fmt/vet/test/lint、OpenAPI 文档 lint、sqlc/OpenAPI 生成物漂移和 Docker build，并已将官方 `actions/checkout` / `actions/setup-go` / `actions/setup-node` 使用 Node 24 兼容主版本，不做发布部署或镜像推送。参考 design/implementation 016、019、020、027，ADR-0020、0021、0022、0023。 |
