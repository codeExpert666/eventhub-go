...
| HTTP DTO / Java VO 对照 | Java Request DTO、Response DTO、VO 命名习惯 | Go `internal/http/dto` 统一承载 HTTP request/response；不设置 `internal/http/vo`；`internal/http/response` 只承载 `APIResponse` envelope 和 writer；domain value object 放 `internal/domain` | 已对齐 | Go 版不逐字照搬 Java VO 命名；system `EchoRequest`、`PingResponse`、`EchoResponse`、`HealthResponse`、`InfoResponse` 已迁入 `internal/http/dto`。Java 中 View Object 语义在 Go 版归入 HTTP DTO，DDD Value Object 归入 domain，避免 View Object 和 Value Object 命名歧义。参考 `docs/ai/design/003-http-dto-boundary.md`、`docs/ai/adr/0006-http-dto-vs-vo-boundary.md`、`docs/ai/implementation/003-http-dto-boundary.md`。 |
