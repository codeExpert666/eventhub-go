...
| requestId、配置与结构化日志 | `infra/logging/RequestIdFilter.java`、`application*.yml`、Logback MDC | `internal/platform/idgen`、`internal/http/middleware/request_id.go`、`internal/config`、`internal/platform/log`、`internal/platform/db`、`internal/platform/redis`、`configs/*.env.example` | 已对齐 | 合法 `X-Request-Id` 复用，非法或缺失时重建；写入响应头、context、统一响应体和 `slog` 字段。Go 当前覆盖 app/env/port/version/log，并已新增数据库与 Redis 平台连接底座；数据库、Redis、JWT 的启动配置装配待后续业务接入。 |
...
| refresh token 与 auth session 服务流程 | `AuthTokenProperties.java`、`AuthSessionService*`、`RefreshTokenHasher.java`、`RefreshTokenParser.java`、`AuthServiceImpl.java`、auth integration tests | 未来 `internal/security/refresh`、`internal/service/auth`、`internal/http/handler/auth`、`internal/http/dto/auth` | 待迁移 | 持久化底座已对齐，但 Go 尚未实现 opaque refresh token 生成/解析、哈希服务、登录创建会话、refresh 轮换服务、logout、重放/过期业务错误和 HTTP 契约。 |
