...
Go 版 service package 内部采用同 package 多文件拆分，而不是照搬 Java 的单 Service 类结构，也不新增 `internal/service/<domain>/dto` 子包。

`internal/service/<domain>/service.go` 保留 `Service` struct、constructor 和依赖字段；写操作输入放 `command.go`，命名为 `XxxCommand`；读操作、列表、搜索、详情输入放 `query.go`，命名为 `XxxQuery`；service 输出放 `result.go`，命名为 `XxxResult` 或窄范围内部结果类型；真正业务方法按 use case 拆到 `register.go`、`login.go`、`create_event.go`、`reserve_ticket.go` 等文件。没有真实 Query 或 errors 类型时，不创建空 `query.go` 或 `errors.go`。

service Command / Query / Result 不带 HTTP `json` tag，不使用 `XxxRequest`、`XxxResponse`、`XxxDTO`、`XxxVO`、`XxxResp` 等 HTTP 或 Java 命名后缀；service 不依赖 `internal/http/dto`，也不向 handler 暴露 sqlc generated model。handler 负责 DTO 与 service contract 的映射。

2026-06-02 的 service contract 边界调整确认：`internal/service/system` 已拆为 `service.go`、`command.go`、`result.go`、`ping.go`、`echo.go`、`actuator.go`，作为后续 auth/user/event/order 等复杂模块的最小样板。
