# ADR：golangci-lint 质量门禁固定版本并提供 Docker fallback

## 标题
使用固定版本 golangci-lint，并在本机未安装时通过 Docker 镜像执行 lint
...
- `.golangci.yml` 使用低噪音规则集：
  ...
  - `staticcheck`
  - `unused`
- `.golangci.yml` 使用 `disable-all: true`，只显式开启当前阶段需要的规则。
- Makefile `lint` 优先执行本机 `golangci-lint run ./...`。
- 如果本机没有 `golangci-lint`，Makefile 使用固定 Docker 镜像 `golangci/golangci-lint:v1.64.8`。
